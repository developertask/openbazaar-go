...
func newJSONAPIHandler(node *core.developertaskNode, authCookie http.Cookie, config schema.APIConfig) *jsonAPIHandler {
...
func (i *jsonAPIHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if i.config.Authenticated {
		if i.config.Username == "" || i.config.Password == "" {
			cookie, err := r.Cookie("developertask_Auth_Cookie")
			if err != nil {
				w.WriteHeader(http.StatusForbidden)
				fmt.Fprint(w, "403 - Forbidden")
				return
