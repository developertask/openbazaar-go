...
import (
	...
	"github.com/developertask/openbazaar-go/schema"
	sto "github.com/developertask/openbazaar-go/storage"
	"github.com/developertask/openbazaar-go/storage/dropbox"
...
type Start struct {
	...
	ForceKeyCachePurge bool `long:"forcekeypurge" description:"repair test for issue developertask/openbazaar-go#1593; use as instructed only"`
}
...
func (x *Start) Execute(args []string) error {
	...
	core.Node = &core.developertaskNode{
	...
	go func() {
		...
		log.Info("developertask Service Ready")
...
func newHTTPGateway(node *core.developertaskNode, ctx commands.Context, authCookie http.Cookie, config schema.APIConfig, noLogFiles bool) (*api.Gateway, error) {
...
func printSplashScreen(verbose bool) {
	...
	white.DisableColor()
	fmt.Println("")
	fmt.Println("developertask Server v" + core.VERSION)
	log.Infof("\ndevelopertask Server v%s", core.VERSION)
	if !verbose {
