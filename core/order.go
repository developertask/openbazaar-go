...
func (n *developertaskNode) GetOrder(orderID string) (*pb.OrderRespApi, error) {
	var (
		err      error
		isSale   bool
...
func (n *developertaskNode) Purchase(data *repo.PurchaseData) (orderID string, paymentAddress string, paymentAmount *repo.CurrencyValue, vendorOnline bool, err error) {
...
func prepareModeratedOrderContract(data *repo.PurchaseData, n *developertaskNode, contract *pb.RicardianContract, wal wallet.Wallet) (*pb.RicardianContract, error) {
...
func processOnlineDirectOrder(resp *pb.Message, n *developertaskNode, wal wallet.Wallet, contract *pb.RicardianContract) (string, string, big.Int, bool, error) {
	// Vendor responded
...
func processOfflineDirectOrder(n *developertaskNode, wal wallet.Wallet, contract *pb.RicardianContract, payment *pb.Order_Payment) (string, string, big.Int, error) {
	// Vendor offline
...
func processOnlineModeratedOrder(resp *pb.Message, n *developertaskNode, contract *pb.RicardianContract) (string, string, big.Int, bool, error) {
	// Vendor responded
	if resp.MessageType == pb.Message_ERROR {
		return "", "", *big.NewInt(0), false, extractErrorMessage(resp)
	}
...
func processOfflineModeratedOrder(n *developertaskNode, contract *pb.RicardianContract) (string, string, big.Int, error) {
	// Vendor offline
	// Send using offline messaging
	log.Warningf("Vendor %s is offline, sending offline order message", contract.VendorListings[0].VendorID.PeerID)
	peerID, err := peer.IDB58Decode(contract.VendorListings[0].VendorID.PeerID)
...
func (n *developertaskNode) createContractWithOrder(data *repo.PurchaseData) (*pb.RicardianContract, error) {
	var (
...
func getListing(n *developertaskNode, contract *pb.RicardianContract, item repo.Item) (*repo.Listing, error) {
...
func getRatingKeysForOrder(data *repo.PurchaseData, n *developertaskNode, ts *timestamp.Timestamp) ([][]byte, error) {
	var ratingKeys [][]byte
...
func (n *developertaskNode) currencyInAcceptedCurrenciesList(checkCode string, acceptedCurrencies []string) bool {
...
func (n *developertaskNode) EstimateOrderTotal(data *repo.PurchaseData) (*big.Int, error) {
	contract, err := n.createContractWithOrder(data)
	if err != nil {
		return big.NewInt(0), err
	}
...
func (n *developertaskNode) CancelOfflineOrder(contract *pb.RicardianContract, records []*wallet.TransactionRecord) error {
	orderID, err := n.CalcOrderID(contract.BuyerOrder)
	if err != nil {
		return err
	}
...
func (n *developertaskNode) CalcOrderID(order *pb.Order) (string, error) {
...
func (n *developertaskNode) CalculateOrderTotal(contract *pb.RicardianContract) (*big.Int, error) {
	var (
		total         = big.NewInt(0)
...
func (n *developertaskNode) calculateShippingTotalForListings(contract *pb.RicardianContract, listings map[string]*repo.Listing) (*big.Int, error) {
...
func (n *developertaskNode) ValidateOrder(contract *pb.RicardianContract, checkInventory bool) error {
...
collectListings:
	...
	for _, item := range contract.BuyerOrder.Items {
		...
		if q := GetOrderQuantity(listingMap[item.ListingHash], item); q.IsInt64() {
			...
			// TODO: https://github.com/developertask/openbazaar-go/issues/1739
...
func (n *developertaskNode) hasKnownListings(contract *pb.RicardianContract) bool {
	for _, listing := range contract.VendorListings {
...
func (n *developertaskNode) ValidateDirectPaymentAddress(order *pb.Order) error {
...
func (n *developertaskNode) ValidateModeratedPaymentAddress(order *pb.Order, timeout time.Duration) error {
	wal, err := n.Multiwallet.WalletForCurrencyCode(order.Payment.AmountCurrency.Code)
	if err != nil {
		return err
	}
...
func (n *developertaskNode) SignOrder(contract *pb.RicardianContract) (*pb.RicardianContract, error) {
...
func (n *developertaskNode) ValidatePaymentAmount(requestedAmount, paymentAmount *big.Int) bool {
...
func (n *developertaskNode) ReserveCurrencyConverter() (*repo.CurrencyConverter, error) {
