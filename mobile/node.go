...
import (
	...
	"github.com/developertask/openbazaar-go/api"
	"github.com/developertask/openbazaar-go/core"
	"github.com/developertask/openbazaar-go/ipfs"
	obnet "github.com/developertask/openbazaar-go/net"
	rep "github.com/developertask/openbazaar-go/net/repointer"
	ret "github.com/developertask/openbazaar-go/net/retriever"
	"github.com/developertask/openbazaar-go/net/service"
	"github.com/developertask/openbazaar-go/repo"
	"github.com/developertask/openbazaar-go/repo/db"
	apiSchema "github.com/developertask/openbazaar-go/schema"
	"github.com/developertask/openbazaar-go/storage/selfhosted"
	"github.com/developertask/openbazaar-go/wallet"
	lis "github.com/developertask/openbazaar-go/wallet/listeners"
	"github.com/developertask/openbazaar-go/wallet/resync"
...
type Node struct {
	developertaskNode *core.developertaskNode
	config         NodeConfig
...
func NewNodeWithConfig(config *NodeConfig, password string, mnemonic string) (*Node, error) {
	...
	// developertask node setup
	node := &core.developertaskNode{
		BanManager:                    bm,
		Datastore:                     sqliteDB,
	...
	return &Node{developertaskNode: node, config: *config, ipfsConfig: ncfg, apiConfig: apiConfig, startMtx: sync.Mutex{}}, nil
}
...
// Start start openbazaard (developertask daemon)
...
func (n *Node) start() error {
	...
	n.developertaskNode.IpfsNode = nd
	n.developertaskNode.DHT = dhtRouting
	...
	if err != nil {
		...
		n.developertaskNode.RootHash = string(rec.Value)
	}

	configFile, err := ioutil.ReadFile(path.Join(n.developertaskNode.RepoPath, "config"))
	if err != nil {
		return err
	}
	...
	n.developertaskNode.MessageStorage = selfhosted.NewSelfHostedStorage(n.developertaskNode.RepoPath, n.developertaskNode.IpfsNode, n.developertaskNode.PushNodes, n.developertaskNode.SendStore)
	...
	n.developertaskNode.Pubsub = ps

	// Start gateway
	// Create authentication cookie
	var authCookie http.Cookie
	authCookie.Name = "developertask_Auth_Cookie"
	...
	go func() {
		resyncManager := resync.NewResyncManager(n.developertaskNode.Datastore.Sales(), n.developertaskNode.Datastore.Purchases(), n.developertaskNode.Multiwallet)
		if !n.config.DisableWallet {
			if resyncManager == nil {
				n.developertaskNode.WaitForMessageRetrieverCompletion()
			}
			TL := lis.NewTransactionListener(n.developertaskNode.Multiwallet, n.developertaskNode.Datastore, n.developertaskNode.Broadcast)
			for ct, wal := range n.developertaskNode.Multiwallet {
				WL := lis.NewWalletListener(n.developertaskNode.Datastore, n.developertaskNode.Broadcast, ct)
				wal.AddTransactionListener(WL.OnTransactionReceived)
				wal.AddTransactionListener(TL.OnTransactionReceived)
			}
			su := wallet.NewStatusUpdater(n.developertaskNode.Multiwallet, n.developertaskNode.Broadcast, n.developertaskNode.IpfsNode.Context())
			go su.Start()
			go n.developertaskNode.Multiwallet.Start()
			if resyncManager != nil {
		...
		n.developertaskNode.Service = service.New(n.developertaskNode, n.developertaskNode.Datastore)
		n.developertaskNode.Service.WaitForReady()
		MR := ret.NewMessageRetriever(ret.MRConfig{
			Db:        n.developertaskNode.Datastore,
			IPFSNode:  n.developertaskNode.IpfsNode,
			DHT:       n.developertaskNode.DHT,
			BanManger: n.developertaskNode.BanManager,
			Service:   n.developertaskNode.Service,
			PrefixLen: 14,
			PushNodes: n.developertaskNode.PushNodes,
			Dialer:    nil,
			SendAck:   n.developertaskNode.SendOfflineAck,
			SendError: n.developertaskNode.SendError,
		})
		go MR.Run()
		n.developertaskNode.MessageRetriever = MR
		PR := rep.NewPointerRepublisher(n.developertaskNode.DHT, n.developertaskNode.Datastore, n.developertaskNode.PushNodes, n.developertaskNode.IsModerator)
		go PR.Run()
		n.developertaskNode.PointerRepublisher = PR
		MR.Wait()

		n.developertaskNode.PublishLock.Unlock()
		publishUnlocked = true
		err = n.developertaskNode.UpdateFollow()
		if err != nil {
			log.Error(err)
		}
		if !n.developertaskNode.InitalPublishComplete {
			err = n.developertaskNode.SeedNode()
			if err != nil {
				log.Error(err)
			}
		}
		n.developertaskNode.SetUpRepublisher(republishInterval)
	}()
	n.started = true
	return nil
}
...
func (n *Node) stop() error {
	...
	n.developertaskNode.Datastore.Close()
	repoLockFile := filepath.Join(n.developertaskNode.RepoPath, fsrepo.LockFile)
	if err := os.Remove(repoLockFile); err != nil {
		log.Error(err)
	}
	n.developertaskNode.Multiwallet.Close()
	if err := n.developertaskNode.IpfsNode.Close(); err != nil {
		log.Error(err)
	}
...
func (n *Node) Restart() error {
	...
	n.developertaskNode = newNode.developertaskNode
...
func newHTTPGateway(node *Node, ctx commands.Context, authCookie http.Cookie, config apiSchema.APIConfig) (*api.Gateway, error) {
	...
	return api.NewGateway(node.developertaskNode, authCookie, manet.NetListener(gwLis), config, mainLoggingBackend, opts...)
}
