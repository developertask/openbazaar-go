...
type developertaskService struct {
	host      host.Host
	self      peer.ID
...
func New(node *core.developertaskNode, datastore repo.Datastore) *developertaskService {
	service := &developertaskService{
	...
	log.Infof("developertask service running at %s", ipfs.IPFSProtocolAppMainnetOne)
	return service
}

func (service *developertaskService) WaitForReady() {
	<-service.node.DHT.BootstrapChan
}

func (service *developertaskService) DisconnectFromPeer(p peer.ID) {
...
func (service *developertaskService) HandleNewStream(s inet.Stream) {
	go service.handleNewMessage(s)
}

func (service *developertaskService) handleNewMessage(s inet.Stream) {
	defer s.Close()
...
func (service *developertaskService) SendRequest(ctx context.Context, p peer.ID, pmes *pb.Message) (*pb.Message, error) {
...
func (service *developertaskService) SendMessage(ctx context.Context, p peer.ID, pmes *pb.Message) error {
