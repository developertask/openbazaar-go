...
class developertaskTestFramework(object):
    ...
    def wait_for_init_success(process):
        while True:
            if process.poll() is not None:
                raise Exception("developertask node initialization failed")
            output = process.stdout
            for o in output:
                if "developertask repo initialized" in str(o):
                    return
    ...
    def wait_for_start_success(process, node):
        ...
        while True:
            if process.poll() is not None:
                raise Exception("developertask node failed to start")
            output = process.stdout
            for o in output:
                if "Peer ID:" in str(o):
    ...
    def main(self, options=["--disablewallet", "--testnet", "--disableexchangerates"]):
        parser = argparse.ArgumentParser(
                    description="developertask Test Framework",
