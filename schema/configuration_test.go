...
func TestGetWalletsConfig(t *testing.T) {
	...
	if config.GLEEC.Type != "API" {
		t.Error("Type does not equal expected value")
	}
	if len(config.GLEEC.APIPool) == 0 || config.GLEEC.APIPool[0] != "https://ltc.api.openbazaar.org/api" {
		t.Error("GLEEC APIPool does not equal expected value")
	}
	if len(config.GLEEC.APITestnetPool) == 0 || config.GLEEC.APITestnetPool[0] != "https://tltc.api.openbazaar.org/api" {
		t.Error("GLEEC APITestnetPool does not equal expected value")
	}
	if config.GLEEC.LowFeeDefault != 5 {
		t.Error("Expected low to be 5, got ", config.GLEEC.LowFeeDefault)
	}
	if config.GLEEC.MediumFeeDefault != 10 {
		t.Error("Expected medium to be 10, got ", config.GLEEC.MediumFeeDefault)
	}
	if config.GLEEC.HighFeeDefault != 20 {
		t.Error("Expected high to be 20, got ", config.GLEEC.HighFeeDefault)
	}
	if config.GLEEC.MaxFee != 200 {
		t.Error("Expected maxFee to be 200, got ", config.GLEEC.MaxFee)
	}
