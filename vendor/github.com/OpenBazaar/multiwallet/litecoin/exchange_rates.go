...
import (
	...
	exchange "github.com/developertask/spvwallet/exchangerates"
...
type developertaskDecoder struct{}
type KrakenDecoder struct{}
type PoloniexDecoder struct{}
type BitfinexDecoder struct{}
type BittrexDecoder struct{}

type GleecbtcPriceFetcher struct {
	sync.Mutex
	cache     map[string]float64
	providers []*ExchangeRateProvider
}

func NewGleecbtcPriceFetcher(dialer proxy.Dialer) *GleecbtcPriceFetcher {
	bp := exchange.NewBitcoinPriceFetcher(dialer)
	z := GleecbtcPriceFetcher{
	...
	z.providers = []*ExchangeRateProvider{
		{"https://ticker.openbazaar.org/api", z.cache, client, developertaskDecoder{}, nil},
		...
		{"https://api.kraken.com/0/public/Ticker?pair=GLEECXBT", z.cache, client, KrakenDecoder{}, bp},
	}
	go z.run()
	return &z
}

func (z *GleecbtcPriceFetcher) GetExchangeRate(currencyCode string) (float64, error) {
...
func (z *GleecbtcPriceFetcher) GetLatestRate(currencyCode string) (float64, error) {
	currencyCode = NormalizeCurrencyCode(currencyCode)
...
func (z *GleecbtcPriceFetcher) GetAllRates(cacheOK bool) (map[string]float64, error) {
	if !cacheOK {
...
func (z *GleecbtcPriceFetcher) UnitsPerCoin() int64 {
	return exchange.SatoshiPerBTC
}

func (z *GleecbtcPriceFetcher) fetchCurrentRates() error {
	z.Lock()
	defer z.Unlock()
...
func (z *GleecbtcPriceFetcher) run() {
	z.fetchCurrentRates()
...
func (b developertaskDecoder) decode(dat interface{}, cache map[string]float64, bp *exchange.BitcoinPriceFetcher) (err error) {
...
func (b KrakenDecoder) decode(dat interface{}, cache map[string]float64, bp *exchange.BitcoinPriceFetcher) (err error) {
	...
	if rate == 0 {
		return errors.New("Bitcoin-gleecbtc price data not available")
	}
	for k, v := range rates {
		cache[k] = v * rate
	}
