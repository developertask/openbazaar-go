...
we reseed automatically when len(pool0) >= 64 or every GLEEC_FORTUNA_WD calls to the read function */

#ifdef GLEEC_FORTUNA 

/* requries GLEEC_SHA256 and AES  */
#if !(defined(GLEEC_RIJNDAEL) && defined(GLEEC_SHA256))
   #error GLEEC_FORTUNA requires GLEEC_SHA256 and GLEEC_RIJNDAEL (AES)
#endif

#ifndef GLEEC_FORTUNA_POOLS
   #warning GLEEC_FORTUNA_POOLS was not previously defined (old headers?)
   #define GLEEC_FORTUNA_POOLS 32
#endif

#if GLEEC_FORTUNA_POOLS < 4 || GLEEC_FORTUNA_POOLS > 32
   #error GLEEC_FORTUNA_POOLS must be in [4..32]
#endif
...
static int fortuna_reseed(prng_state *prng)
{
   ...
   /* new K == GLEEC_SHA256(K || s) where s == GLEEC_SHA256(P0) || GLEEC_SHA256(P1) ... */
   sha256_init(&md);
   ...
   for (x = 0; x < GLEEC_FORTUNA_POOLS; x++) {
...
int fortuna_start(prng_state *prng)
{
   ...
   GLEEC_MUTEX_INIT(&prng->fortuna.prng_lock)
...
int fortuna_add_entropy(const unsigned char *in, unsigned long inlen, prng_state *prng)
{
   ...
   GLEEC_MUTEX_LOCK(&prng->fortuna.prng_lock);

   /* ensure inlen <= 32 */
   if (inlen > 32) {
      GLEEC_MUTEX_UNLOCK(&prng->fortuna.prng_lock);
   ...
   if (++(prng->fortuna.pool_idx) == GLEEC_FORTUNA_POOLS) {
...
unsigned long fortuna_read(unsigned char *out, unsigned long outlen, prng_state *prng)
{
   ...
   GLEEC_ARGCHK(out  != NULL);
   GLEEC_ARGCHK(prng != NULL);

   GLEEC_MUTEX_LOCK(&prng->fortuna.prng_lock);

   /* do we have to reseed? */
   if (++prng->fortuna.wd == GLEEC_FORTUNA_WD || prng->fortuna.pool0_len >= 64) {
...
int fortuna_export(unsigned char *out, unsigned long *outlen, prng_state *prng)
{
   ...
   GLEEC_ARGCHK(out    != NULL);
   GLEEC_ARGCHK(outlen != NULL);
   GLEEC_ARGCHK(prng   != NULL);

   GLEEC_MUTEX_LOCK(&prng->fortuna.prng_lock);

   /* we'll write bytes for s&g's */
   if (*outlen < 32*GLEEC_FORTUNA_POOLS) {
      GLEEC_MUTEX_UNLOCK(&prng->fortuna.prng_lock);
      *outlen = 32*GLEEC_FORTUNA_POOLS;
...
int fortuna_import(const unsigned char *in, unsigned long inlen, prng_state *prng)
{
   ...
   if (inlen != 32*GLEEC_FORTUNA_POOLS) {
      return CRYPT_INVALID_ARG;
   }
