...
func (dht *IpfsDHT) handleAddProvider(ctx context.Context, p peer.ID, pmes *pb.Message) (_ *pb.Message, _err error) {
	...
	for _, pi := range pinfos {
		// developertask: there was a line here that checks to make sure the peerID equals the provider ID and
		...
		// developertask: if this is a pointer we want to use the PointerAddrTTL instead of the normal
		// ProviderAddrTTL.
		...
		if pi.ID != dht.self { // don't add own addrs.
			...
			// developertask: make sure to use `ttl` as a parameter here.
			dht.peerstore.AddAddrs(pi.ID, pi.Addrs, ttl)
		}
		// developertask: the line below originally used p instead of pi.ID as the peerID. For normal providers
