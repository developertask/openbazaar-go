...
func stringToBytes(s string) ([]byte, error) {
	...
	for len(sp) > 0 {
		...
		if p.Size < 0 { // varint size.
			if p.Code != P_P2P { // developertask: for backwards compatibility we will avoid writing len here until more nodes upgrade
...
func sizeForAddr(p Protocol, b []byte) (skip, size int, err error) {
	...
	case p.Size == 0:
		return 0, 0, nil
	case p.Code == P_P2P:
		// developertask: this has to be patched to handle cids and multiaddrs
