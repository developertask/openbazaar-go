...
import (
	...
	mwConfig "github.com/developertask/multiwallet/config"
...
func NewMultiWallet(cfg *WalletConfig) (multiwallet.MultiWallet, error) {
	...
	if cfg.ConfigFile.GLEEC != nil && cfg.ConfigFile.GLEEC.Type == "API" {
		enableAPIWallet[wallet.Gleecbtc] = cfg.ConfigFile.GLEEC
	}
...
func createAPIWallet(coin wallet.CoinType, coinConfigOverrides *schema.CoinConfig, cfg *WalletConfig) (wallet.CoinType, wallet.Wallet, error) {
	...
	case wallet.Gleecbtc:
		if testnet {
			actualCoin = wallet.TestnetGleecbtc
		} else {
			actualCoin = wallet.Gleecbtc
		}
		w, err := gleecbtc.NewGleecbtcWallet(*coinConfig, cfg.Mnemonic, cfg.Params, cfg.Proxy, cache.NewMockCacher(), cfg.DisableExchangeRates)
		if err != nil {
			return InvalidCoinType, nil, err
		}
		return actualCoin, w, nil
...
func prepareAPICoinConfig(coin wallet.CoinType, override *schema.CoinConfig, walletConfig *WalletConfig) *mwConfig.CoinConfig {
	...
	case wallet.Gleecbtc:
		defaultConfig = defaultConfigSet.GLEEC
	case wallet.Zcash:
